...
module.exports = (function(){
    ...
    AmazonRSS.getDealsFeedFromScripts = function(scripts) {
        ...
        scripts.forEach(function(widget) {
            ...
            for ( let key in deals.dealDetails ) {
                ...
                let deal_time = date_timestamp + parseFloat(deals.dealDetails[key].msToStart);

                deal_time = new Date(deal_time);

                if ( !deals.dealDetails.hasOwnProperty(key) ) continue;

                data.items.push({
                    title: deals.dealDetails[key].title,
                    link: Util.addQueryParameters(deals.dealDetails[key].egressUrl, {
                        tag: process.env.affiliate_id
                    }),
                    description: deals.dealDetails[key].description,
                    image: deals.dealDetails[key].primaryImage,
                    list_price: deals.dealDetails[key].primaryImage,
                    deal_price: null,
                    savings: null,
                    is_dealoftheday: null,
                    pub_date: Util.rssDate(deal_time),
                });
    ...
    AmazonRSS.keywordifyItems = function(items, list, symbol) {
        ...
        // Set the list to lowercase so we don't have to worry about
        // case for matches
