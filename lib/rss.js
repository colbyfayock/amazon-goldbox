...
class Rss {
  ...
  constructor(data) {
    ...
    this.xmlns_original = 'http://givemecheapstuff.com.s3-website-us-east-1.amazonaws.com/original/';
  ...
  ingestXml(data) {
    return new Promise((resolve, reject) => {
      parseString(data, (error, result = {}) => {
        ...
        if ( result.rss.channel[0] && Array.isArray(result.rss.channel[0].item) ) {
  ...
  toXml() {
    ...
    const xml = {
      'rss': {
        ...
        '@xmlns:original': this.xmlns_original,
        'channel' : {
          'title': {
            '#text': this.title,
          },
