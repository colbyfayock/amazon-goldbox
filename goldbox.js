...
module.exports.goldbox = function(event, context) {
  ...
  axios.get(process.env.rss_feed)
    .then(response => {

      Kenny.log(`RSS Success - ${process.env.rss_feed}`);

      const widget_scripts = AmazonRSS.getScriptsFromHtml(response.data);
      const deals = AmazonRSS.getDealsFeedFromScripts(widget_scripts, {
    ...
    .catch(error => {
      ...
      Util.respond({
        callback: event,
        status_code: 500,
        message: Kenny.log(`RSS Error - ${error}`)
      });
